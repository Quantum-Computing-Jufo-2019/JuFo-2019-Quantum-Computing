...
void setup() {
  ...
  minus=new Picture(loadImage("minus.png"), new PVector(width/6, height/5), width/15);
  plus=new Picture(loadImage("plus.png"), new PVector(width/6*2, height/5), width/15);
  erzeugeFelder();
...
void maleKosten() {
  ...
  if (kosten==n*(-2)) {
    ...
    text("Glueckwunsch!\nrichtige Loesung!", (width/4)*0.25, (height/5)*2, width/2-width/10, height-height/6);//width/2+width/20
    textSize(64*displayDensity);
    text("Kosten: "+ n*(-2), (width/4), (height/5)*3.5);
    //wenn Lösung nicht richtig
  } else {
    noStroke();
    fill(255, 90, 90, 140);
    //rotes Rechteck
    rect((width/4)*0.25, (height/5)*2, width/2-width/10, height-height/2, 63);
    fill(0);
    textSize(83*displayDensity);
    text("Kosten:", (width/4), (height/5)*2.6);
    textSize(95*displayDensity);
    text(kosten, (width/16)*3, (height/5)*3.8); 
    textAlign(CENTER, TOP);
    //optimale Kosten (nach rechts gerückt wenn Kosten 3 stellig)
    if (kosten<99) {
      textSize(27*displayDensity);
      text("optimal: "+n*(-2), (width/16)*5.5, (height/5)*3.9);
    } else {
      textSize(27*displayDensity);
      text("optimal: "+n*(-2), (width/16)*5.9, (height/5)*3.9);
    }
    stroke(0);
  }
  fill(255);
...
void erzeugeFelder() {
  ...
  for (int i=0; i<n; i++) {
    for (int j=0; j<n; j++) {
      int xPos=width/2+width/20+groesse*i; //=> abstand zu Rand + breite jedes Feldes * Nummer des Feldes
